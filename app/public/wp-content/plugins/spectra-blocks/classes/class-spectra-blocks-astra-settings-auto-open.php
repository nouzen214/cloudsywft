<?php
/**
 * Spectra Blocks Astra Settings Auto Open
 * Automatically opens the Astra Settings panel for the Learn "Page Layout" steps.
 *
 * Ported from UAGB's UAGB_Astra_Settings_Auto_Open. The Learn "Choose Page Layout"
 * and "Show or Hide Elements" steps redirect the editor to `#astra-container-layout`
 * / `#astra-disable-elements`; this class listens for those hashes, opens the Astra
 * Settings sidebar panel, and scrolls to / highlights the relevant section.
 *
...
if ( ! class_exists( 'Spectra_Blocks_Astra_Settings_Auto_Open' ) ) {

	/**
	 * Class Spectra_Blocks_Astra_Settings_Auto_Open
	 *
	 * @since 1.0.4
	 */
	class Spectra_Blocks_Astra_Settings_Auto_Open {
		...
		public function enqueue_auto_open_script() {
			...
			// The Astra Settings panel only exists when the Astra theme is active.
			...
			( function() {
				...
				if (window.spectraBlocksAstraAutoOpenExecuted) {
					return;
				}
				window.spectraBlocksAstraAutoOpenExecuted = true;
									...
									if (!targetElement) {
										...
										for (var i = 0; i < headings.length; i++) {
											...
											if (heading.textContent && heading.textContent.toLowerCase().includes('container')) {
												...
												highlightElement(targetElement, 3000, '" . esc_js( __( 'Choose page layout using Astra Container Layout Settings.', 'spectra-blocks' ) ) . "');
												break;
											}
										}
									}
							...
							if (targetElement) {
								...
								setTimeout(() => {
									...
									if( 'Disable Elements' === sectionName ) {
										highlightElement(targetElement, 3000, '" . esc_js( __( 'Show and hide elements using Astra Disable Elements Settings.', 'spectra-blocks' ) ) . "');
									}
								}, 1000);
							} else {
				...
				function highlightElement(element, duration, tooltipText) {
					...
					// Add tooltip CSS if not exists (shared id with the main learn-actions handler)
	...
	Spectra_Blocks_Astra_Settings_Auto_Open::get_instance();
}
